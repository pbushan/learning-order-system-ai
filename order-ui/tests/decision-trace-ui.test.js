...
test('normalizeEvent and timeline rendering replace blank summary text with placeholder', () => {
  assert.equal(ui.toDisplayText(null), 'No details available');
  assert.equal(ui.toDisplayText(undefined), 'No details available');
  assert.equal(ui.toDisplayText(''), 'No details available');
  assert.equal(ui.toDisplayText('   '), 'No details available');
  assert.equal(ui.toDisplayText('Actual details'), 'Actual details');

  const normalized = ui.normalizeEvent({
    traceId: 'trace-2',
    sessionId: 'session-2',
  ...
  assert.equal(normalized.summary, 'No details available');
  ...
  assert.equal(timeline[0].summary, 'No details available');
});
