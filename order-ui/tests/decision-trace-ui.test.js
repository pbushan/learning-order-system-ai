...
test('normalizeEvent replaces blank summary text with a consistent placeholder', () => {
  const normalizedBlank = ui.normalizeEvent({
    eventType: 'intake.session.started',
    summary: '   '
  });
  const normalizedMissing = ui.normalizeEvent({
    eventType: 'intake.session.started'
  });

  assert.equal(normalizedBlank.summary, ui.EMPTY_EVENT_TEXT);
  assert.equal(normalizedMissing.summary, ui.EMPTY_EVENT_TEXT);
});
