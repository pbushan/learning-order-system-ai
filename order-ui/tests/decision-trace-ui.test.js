...
test('buildCompactTraceSummary returns fallback for empty or missing events', () => {
  assert.equal(ui.buildCompactTraceSummary([]), 'No trace events available.');
  assert.equal(ui.buildCompactTraceSummary(), 'No trace events available.');
});

test('buildCompactTraceSummary keeps a short summary for populated events', () => {
  assert.equal(ui.buildCompactTraceSummary([{ eventType: 'intake.session.started' }]), '1 trace event available.');
  assert.equal(ui.buildCompactTraceSummary([{ eventType: 'a' }, { eventType: 'b' }]), '2 trace events available.');
});
